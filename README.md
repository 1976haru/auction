...
### 에이전트 옥션 파인더 v1 — 초보자 친화 홈 화면

첫 화면을 개발자용 분석 대시보드에서 **"모두의경매" 스타일의 모바일 앱 홈**으로 개편했습니다. 20~30대 경매 초보자가 봐도 "AI가 물건을 찾아주고 분석해주는 앱"으로 직관적으로 느끼도록 구성하되, 기존 검색·필터·차트·상세·비교·메모 기능은 하나도 삭제하지 않고 홈 아래로 정리했습니다. PC 에서는 넓은 웹 대시보드, 모바일에서는 앱 홈처럼 반응형으로 동작합니다.

홈 화면 구성 (위→아래):
1. **앱 헤더** — 앱 이름(에이전트 옥션 파인더) · v1 · Mock API 기반 · 실제 입찰 없음 배지
2. **오늘의 경매 현황 카드** — 전체 물건 / 경매 / 공매 / 추천 후보 / 초보자 후보 / 고위험 / 입찰임박 수를 숫자 카드로 한눈에
3. **AI 주요 버튼 2개** — `🔍 AI 물건 검색`(→ AI 에이전트 검색 섹션으로 스크롤·포커스) · `📊 AI 물건 분석`(→ 추천 TOP 5 섹션으로 스크롤)
4. **카테고리 아이콘 메뉴** — 아파트·자동차·오피스텔·주택·상가·토지·중기·기타 (클릭 시 물건종류 필터 적용 후 전체 물건으로 스크롤. 데모 데이터에 없는 종류는 안내 토스트). 추가로 경매뉴스·권리분석·오늘의퀴즈·모의경매·매각결과·경매지도·경매달력·대출계산기 유틸 메뉴를 기존 섹션·기능에 연결
5. **초보자 모드 토글** — ON 시 고위험·복잡 권리(유치권·법정지상권·지분매각·농취증·분묘기지권·선순위임차인) 물건과 시세 신뢰도 낮은 물건을 숨기고 A/B등급 주거용만 표시. 상태는 `auction:beginner:v1` 로 localStorage 저장
6. **오늘 먼저 볼 물건** — "AI가 오늘 먼저 볼 물건을 골랐어요." 초보자 친화 물건 중 예상차익 큰 순 → 입찰기일 임박 순 3건을 추천등급·초보자 라벨·주소·예상 시세차익·조심할 점·상세보기 버튼이 담긴 카드로 표시
7. **용어 쉽게 보기 모달** — 권리분석/오늘의퀴즈 메뉴에서 유치권·법정지상권·대항력·선순위임차인·지분매각·농지취득자격증명·분묘기지권·명도·말소기준권리·매각물건명세서·현황조사서·감정평가서 12개 용어를 초보자 눈높이로 설명(검색 가능)
8. 이하 **기존 검색 / 필터 / 전체 물건 / 통계 / 상세 / 비교 / 도움말** 기능 전부 유지

추천 카드·오늘 먼저 볼 물건에는 추천등급을 초보자 라벨(A=초보자 검토 가능, B=조심해서 검토, C=보류 후보, D=낮은 우선순위, X=초보자 비추천)로 함께 표기하고, `simple_profit_summary`·`simple_risk_summary` 등 쉬운 문구를 노출합니다. 모든 표시는 mock 데이터 기반 데모이며 법률·투자 판단을 단정하지 않습니다.
...
### Phase K — PC 고급 사용성 (`ffc3ec3` ~ `1bcc1bf`)
...
### Phase N — 인앱 알림·복구·UX (`8b305ca` ~ `7773657`)
- **N87 즐겨찾기 임박 배너** — ★ 매물 중 D-3 이내가 있으면 hero 와 검색바 사이에 빨강 띠 자동 노출. "관심 매물만 보기" 액션으로 즉시 favorites 칩 + 기일 임박순 정렬, × 로 세션 동안 닫기. role=status + aria-live 로 스크린리더 자동 안내. 푸시 권한 없이 임박 매물을 놓치지 않게 하는 인앱 알림
- **N94 결과 0건 zero-state** — 단순 안내 대신 적용된 모든 필터를 칩으로 표시 (검색어·지역·종류·구분·등급·위험·유찰·기일·가격·키워드·점수 하한). 각 칩 × 로 단계별 완화, "필터 초기화" / "검색어만 지우기" 단축 액션. 점선 테두리 + 강조 배경
- **N95 카드 밀도 토글** — 전체 물건 헤더 ▦ 버튼으로 표준/조밀 전환. 조밀은 minmax(220px) + 폰트·padding·pill 압축으로 한 화면에 ~1.3배 더 노출. localStorage 영구 저장, 다크 모드/모바일 친화

### Phase O — 스크롤·내비·자동완성·핫키 (`5be1789` ~ `b0f304f`)
- **O96 위로 스크롤 버튼** — 스크롤 600px+ 시 우하단 floating ↑ 등장, 부드러운 맨 위 스크롤. 비교 트레이/선택 바와 자동 충돌 회피, PC Home 키 단축, RAF 기반 스크롤 추적
...
### Phase U — 에이전트 옥션 파인더 v1 (초보자 친화 홈)
- **U116 모바일 앱형 홈 화면** — 첫 화면을 분석 대시보드에서 모바일 앱 홈으로 개편 (앱 헤더·오늘의 경매 현황 카드·AI 버튼 2개·카테고리/유틸 아이콘 메뉴·초보자 모드·오늘 먼저 볼 물건·전체 탐색 CTA). 기존 검색·필터·차트·상세·비교·메모는 홈 아래로 이동, 삭제 없음. PC 는 1200~1280px 중앙 정렬 + 7~8열 grid, 모바일은 4열 grid·44px 터치 영역
- **U117 오늘의 경매 현황 카드** — summary 의 total/auction/public_sale/recommended/beginner_candidate/high_risk/urgent 를 숫자 카드로. `export_static_dashboard.py` 에 `beginner_candidate_items`·`beginner_mode_default` summary 필드 추가
- **U118 AI 버튼 · 카테고리/유틸 라우팅** — AI 물건 검색→AI 에이전트 검색 섹션, AI 물건 분석→추천 TOP 5 섹션 스크롤. 카테고리는 물건종류 필터, 유틸 메뉴(경매달력→입찰기일순 정렬, 경매지도→차트, 모의경매/대출계산기→추천, 권리분석/오늘의퀴즈→용어 모달 등)를 기존 기능에 연결
- **U119 초보자 모드** — 고위험·복잡 권리·저신뢰 물건 제외 후 A/B 주거용만 표시, `auction:beginner:v1` localStorage 저장. `applyFilters` 에 초보자 필터 통합
- **U120 오늘 먼저 볼 물건 + 쉬운 문구** — 초보자 친화 물건 3건을 추천등급·초보자 라벨·예상 시세차익·조심할 점·상세보기 카드로. 추천 카드에도 초보자 라벨 + `simple_profit_summary`/`simple_risk_summary` 노출. item 레벨 `beginner_friendly`·`beginner_reason`·`simple_*`·`easy_explanation`·`glossary_terms` 필드 추가
- **U121 용어 쉽게 보기 모달** — 권리분석 12개 용어를 초보자 눈높이 설명으로, 검색 가능한 모달 제공

### 누적 사용자 데이터 키
| 키 | 내용 | 백업 포함 |
|---|---|---|
| `auction:favorites:v1` | ★ 관심 매물 ID 셋 | ✅ |
| `auction:beginner:v1` | 🎓 초보자 모드 ON/OFF | ❌ (UI 환경설정) |
...
| `auction:density:v1` | 카드 표준/조밀 모드 선택 | ❌ (UI 환경설정) |
